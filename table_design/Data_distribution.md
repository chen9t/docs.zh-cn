...
并行扫描的开启方式： SET GLOBAL enable_tablet_internal_parallel;。
建议分桶数量不要超过 BE 数量。如果按照上述方式计算的分桶数量远远超过 BE 数量，则建议您适当增加机器资源，否则会导致一个 BE 上 Tablet 数量过多，会导致读写放大都会加大，从而影响集群性能。
